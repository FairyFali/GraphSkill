...
def retrieve_documentation_chapter(
    ...
    if chapters_and_descs and not explored_choices:
        ...
        prompt = (
            f"{instrcution}\n"
            f"Task: Select the chapter keys that most directly help solve the user query.\n\n"
            f"User Query:\n{query}\n\n"
            "Step 1: Determine the most appropriate NetworkX function required to solve the task.\n"
            "Step 2: Based on the selected function, identify the documentation chapter "
            ...
            "3) Disambiguation: when multiple are similar, pick the one most likely to lead to an immediately usable function.\n\n"
            "Output format (STRICT):\n"
            f"- Return ONLY a Python list literal, using double quotes, e.g.: "
            '["key1", "key2", "key3"]\n'
