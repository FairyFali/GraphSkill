"""
Zero-Shot Retrieval Agent + Coding Agent (Unified)

This script implements agentic retrieval-augmented code generation where
an agent determines what documentation to retrieve based on task analysis.

Supports both ComplexGraph and GTools benchmarks via --benchmark flag.

Example Usage:
    # ComplexGraph
    python runners/run_zs_retagent_codingagent.py --benchmark complexgraph --model deepseek-chat --dataset small
    python runners/run_zs_retagent_codingagent.py --benchmark complexgraph --model llama --dataset composite

    # GTools
    python runners/run_zs_retagent_codingagent.py --benchmark gtools --model deepseek-chat --dataset small
    python runners/run_zs_retagent_codingagent.py --benchmark gtools --model llama --dataset large

Dependencies:
    - LLM API keys configured in .env file
    - Dataset files for the chosen benchmark
    - Retrieval utilities for agentic document retrieval
...
try:
    ...
    HAS_RETAGENT = False
    print("⚠ Warning: Retrieval agent not available")


def run_retagent_coding_experiment(
    model_name: str,
    dataset_version: str,
    config: Dict[str, Any],
    ...
    """
    Run retrieval agent + coding agent experiment.
    ...
    """
    print(f"\n{'='*70}")
    print(f"Retrieval Agent + Coding Agent Experiment")
    print(f"Model: {model_name}")
    print(f"Dataset: {config['label']}-{dataset_version.upper()}")
    print(f"{'='*70}\n")

    if not HAS_RETAGENT:
        print("✗ Retrieval agent not available")
        return
    ...
    if output_dir is None:
        ...
        output_dir = Path("LLM_generation_results") / config["output_base"] / "code_generation" / "retagent_codingagent" / dataset_version / model_short
    ...
    for group_idx, question_group in enumerate(questions_data):
        ...
        # Use agentic retrieval to get relevant docs
        try:
            retrieved_docs = retrieval_agent.retrieve(question_text, top_k=3)
        ...
        prompt = create_code_generation_prompt(
            ...
            retrieval_method="Retrieval Agent",
            test_case=test_case
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Retrieval Agent + Coding Agent (unified)",
    ...
    run_retagent_coding_experiment(
