"""
CoT Textual Reasoning (Unified)

This script evaluates LLMs on graph reasoning tasks using Chain-of-Thought textual reasoning
...
Example Usage:
    # ComplexGraph
    python runners/run_cot_textReason.py --benchmark complexgraph --model deepseek-chat --dataset small --max_instances 10
    python runners/run_cot_textReason.py --benchmark complexgraph --model llama --dataset composite --max_instances 10

    # GTools
    python runners/run_cot_textReason.py --benchmark gtools --model deepseek-chat --dataset small --max_instances 10
    python runners/run_cot_textReason.py --benchmark gtools --model llama --dataset large --max_instances 10

    # Evaluate existing results
    python runners/run_cot_textReason.py --benchmark complexgraph --evaluate_only --results_file ./results/all_results.json

Dependencies:
    - LLM API keys configured in .env file
    - Dataset files for the chosen benchmark
    - Few-shot examples in prompts/fs_cot_text_example.txt
...
def load_few_shot_examples(
    ...
    """
    Load universal few-shot Chain-of-Thought (CoT) example from prompts folder.

    The CoT example is universal (task-agnostic) and demonstrates:
    - How to break down complex problems
    - Step-by-step reasoning with explicit intermediate steps
    - Clear explanation of graph structures and properties
    - Logical deduction leading to the final answer

    Args:
        num_shots: Number of examples to include (default: 1)
            Currently supports 1-shot (the example file contains one example)
        examples_path: Custom path to example file
            Default: prompts/fs_cot_text_example.txt

    Returns:
        String containing the CoT few-shot example

    Example format:
        Task Description: [detailed task]
        GRAPH STRUCTURE (Edge List): [edges] Args: [arguments]
        Response: [detailed chain-of-thought reasoning] Final Answer: [answer]

    Raises:
        FileNotFoundError: If example file doesn't exist
    """
    # Set default path to universal CoT example file
    if examples_path is None:
        examples_path = Path("prompts") / "fs_cot_text_example.txt"

    if not examples_path.exists():
        raise FileNotFoundError(f"CoT example file not found: {examples_path}")

    # Read the universal CoT example content
    with open(examples_path, "r", encoding="utf-8") as f:
        cot_example_content = f.read()

    print(f"✓ Loaded universal Chain-of-Thought example from: {examples_path}")

    # Note: This CoT example demonstrates detailed reasoning patterns
    # The same example is used for all tasks to provide consistent guidance

    return cot_example_content
...
def create_reasoning_prompt(question_text: str, edge_list: List[List],
    ...
    The prompt includes:
    1. Few-shot examples (if provided)
    2. Task description from question text
    3. Graph structure in natural language
    4. Task parameters (if any)
    5. Instructions for reasoning without code
    ...
    Args:
        ...
        few_shot_examples: Formatted few-shot examples string (optional)
...
def run_textual_reasoning_experiment(
    ...
    """
    Run CoT textual reasoning experiment.
    ...
    Args:
        ...
        num_shots: Number of CoT examples to include (default: 1)
    """
    print(f"\n{'='*70}")
    print(f"CoT Textual Reasoning Experiment")
    ...
    if output_dir is None:
        ...
        output_dir = Path("LLM_generation_results") / config["output_base"] / "textual_reasoning" / "cot" / dataset_version / model_short
    ...
    for group_idx, question_group in enumerate(questions_data):
        ...
        # Load universal Chain-of-Thought example (same for all tasks)
        # The example is already formatted and ready to use
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CoT textual reasoning evaluation (unified)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""Examples:
    # Basic usage - run experiment with automatic evaluation
    python runners/run_cot_textReason.py --benchmark complexgraph --model gpt-4 --dataset small

    # GTools benchmark
    python runners/run_cot_textReason.py --benchmark gtools --model deepseek-chat --dataset large

    # With custom output directory
    python runners/run_cot_textReason.py --benchmark complexgraph --model gpt-4 --dataset large --output_dir ./results

    # Run specific tasks only
    python runners/run_cot_textReason.py --benchmark complexgraph --model gpt-4 --dataset small --tasks clustering diameter

    # Limit test instances for quick testing
    python runners/run_cot_textReason.py --benchmark complexgraph --model gpt-4 --dataset small --max_instances 5
...
Output Files:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of few-shot examples to include in prompts (default: 1)"
    )
