"""
Build-A-Graph (BAG) Textual Reasoning Baseline (Unified)

This script evaluates LLMs on graph reasoning tasks using direct textual reasoning
with an explicit graph construction instruction. The LLM is prompted to first construct
the graph from the edge list, then reason about it to solve the task.

This variant adds the instruction "Let's construct a graph with the nodes and edges first."
to encourage explicit graph visualization before reasoning.

Supports both ComplexGraph and GTools benchmarks via --benchmark flag.

Example Usage:
    # ComplexGraph
    python runners/run_bag_textReason.py --benchmark complexgraph --model deepseek-chat --dataset small --max_instances 10
    python runners/run_bag_textReason.py --benchmark complexgraph --model llama --dataset composite --max_instances 10

    # GTools
    python runners/run_bag_textReason.py --benchmark gtools --model deepseek-chat --dataset small --max_instances 10
    python runners/run_bag_textReason.py --benchmark gtools --model llama --dataset large --max_instances 10

    # Evaluate existing results
    python runners/run_bag_textReason.py --benchmark complexgraph --evaluate_only --results_file ./results/all_results.json
...
def create_reasoning_prompt(question_text: str, edge_list: List[List],
    ...
    """
    Create a comprehensive prompt for BAG (Build-A-Graph) textual reasoning.

    The prompt includes:
    1. Task description from question text
    2. Graph structure in natural language
    3. Task parameters (if any)
    4. Instructions for explicit graph construction before reasoning
    ...
    # Build the prompt with BAG instruction
    ...
    1. Build the graph structure from the edge list provided
    2. Carefully analyze the constructed graph
    3. Apply graph theory principles to solve the problem
    4. Think step-by-step through your reasoning process
    5. State your final answer explicitly (e.g., "Final Answer: 42" or "Final Answer: [1, 2, 3]")
...
def run_textual_reasoning_experiment(
    ...
    """
    Run BAG (Build-A-Graph) textual reasoning experiment.

    This function:
    1. Loads the dataset (questions and graphs)
    2. For each question group, generates reasoning prompts with BAG instruction
    ...
    """
    print(f"\n{'='*70}")
    print(f"BAG (Build-A-Graph) Textual Reasoning Experiment")
    ...
    # Set up output directory (changed from "zero_shot" to "bag")
    if output_dir is None:
        model_short = model_name.split("/")[-1] if "/" in model_name else model_name
        output_dir = Path("LLM_generation_results") / config["output_base"] / "textual_reasoning" / "bag" / dataset_version / model_short
    ...
    for group_idx, question_group in enumerate(questions_data):
        ...
        for inst_idx, instance in enumerate(tqdm(instances_to_process, desc=f"Processing {task_name}")):
            ...
            # Create prompt with BAG instruction
...
def main():
    ...
    Parses command-line arguments and runs the BAG textual reasoning experiment.
    """
    parser = argparse.ArgumentParser(
        description="BAG (Build-A-Graph) textual reasoning evaluation (unified)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""Examples:
    # Basic usage - run experiment with automatic evaluation
    python runners/run_bag_textReason.py --benchmark complexgraph --model gpt-4 --dataset small

    # GTools benchmark
    python runners/run_bag_textReason.py --benchmark gtools --model deepseek-chat --dataset large

    # With custom output directory
    python runners/run_bag_textReason.py --benchmark complexgraph --model gpt-4 --dataset large --output_dir ./results

    # Run specific tasks only
    python runners/run_bag_textReason.py --benchmark complexgraph --model gpt-4 --dataset small --tasks clustering diameter

    # Limit test instances for quick testing
    python runners/run_bag_textReason.py --benchmark complexgraph --model gpt-4 --dataset small --max_instances 5
