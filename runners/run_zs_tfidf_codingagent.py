"""
Zero-Shot TF-IDF + Coding Agent (Unified)

This script implements retrieval-augmented code generation using TF-IDF
to retrieve relevant documentation, combined with iterative error correction.

Supports both ComplexGraph and GTools benchmarks via --benchmark flag.

Example Usage:
    # ComplexGraph
    python runners/run_zs_tfidf_codingagent.py --benchmark complexgraph --model deepseek-chat --dataset small
    python runners/run_zs_tfidf_codingagent.py --benchmark complexgraph --model llama --dataset composite

    # GTools
    python runners/run_zs_tfidf_codingagent.py --benchmark gtools --model deepseek-chat --dataset small
    python runners/run_zs_tfidf_codingagent.py --benchmark gtools --model llama --dataset large

Dependencies:
    - LLM API keys configured in .env file
    - Dataset files for the chosen benchmark
    - TF-IDF index and documentation retrieval setup
...
try:
    ...
    HAS_TFIDF = False
    print("⚠ Warning: TF-IDF retriever not available")


def run_tfidf_coding_experiment(
    model_name: str,
    dataset_version: str,
    config: Dict[str, Any],
    ...
    """
    Run TF-IDF retrieval + coding agent experiment.
    ...
    """
    print(f"\n{'='*70}")
    print(f"TF-IDF + Coding Agent Experiment")
    ...
    if output_dir is None:
        ...
        output_dir = Path("LLM_generation_results") / config["output_base"] / "code_generation" / "tfidf_codingagent" / dataset_version / model_short
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="TF-IDF + Coding Agent (unified)",
